...
async function execute(cmd) {
  ...
  const existingKeys = (await handler.listVariables()).map(
...
program
  ...
  .option("--token <token>", "Your Gitlab token")
  .option("--key <key>", "Your Gitlab CI variable")
  .option("--value <value>", "Your Gitlab CI value")
  .option(
    "--do-not-force",
